...
export class LockAuthenticator {
    ...
    async validateAndLock(roomId) {
        ...
        try {
            ...
            // Mencegah status 'pending' abadi jika koneksi Firebase dicegat Brave Shields
            const fetchPromise = getDoc(sessionRef);
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error("Timeout: Jaringan diblokir atau tidak merespons.")), 8000);
            });
            ...
            // Semua error, termasuk Timeout 8 detik dari Promise.race, akan dieksekusi di sini
            return this.triggerPhantomRedirect("AKSES DITOLAK ATAU DIHANCURKAN");
        }
    }
