...
export class LockAuthenticator {
    ...
    async validateAndLock(roomId) {
        ...
        try {
            ...
            // TAHAP 3: ANTI-ADMIN BURN (Mencegah hangus jika Admin tidak sengaja klik URL)
            let isAdminBrowser = false;
            try { isAdminBrowser = localStorage.getItem('GODS_EYE_ADMIN') === 'true'; } catch(e) {}

            // Evaluasi aman untuk mengatasi field kosong dari Web Admin (Cegah Crash Rules)
            const isFingerprintEmpty = !data.fingerprint || data.fingerprint.trim() === "";

            if (isFingerprintEmpty) {
                if (!isAdminBrowser) {
                    // Target pertama kali membuka, KUNCI IDENTITAS SEKARANG.
                    await networkTacticalTimeout(updateDoc(sessionRef, { fingerprint: this.fingerprint }), 5000);
                } else {
                    console.log("[GOD'S EYE] Mode Pratinjau Admin Aktif. Identitas perangkat tidak dikunci.");
                }
            } else if (data.fingerprint !== this.fingerprint && !isAdminBrowser) {
                // Identitas beda dengan yang terkunci di database, langsung eksekusi.
            ...
            if (error.message === "NETWORK_BLOCKED_OR_TIMEOUT") {
                ...
                reason = "SISTEM MENDETEKSI ANOMALI IZIN (FIREWALL)";
            }
