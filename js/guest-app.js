...
// QUANTUM AUDIO 2.0 (ELEGAN, TIDAK BERISIK, BERWIBAWA)
class QuantumAudio {
    constructor() { 
    ...
    playBoot() { this._playTone(400, 'sine', 1.0, 0.05, 0.8); } // Hum halus pembukaan
    playDroplet() { this._playTone(880, 'sine', 0.15, 0.03, 0.1); } // Ping senyap saat pesan masuk
    playMechClick() { this._playTone(1200, 'sine', 0.05, 0.02, 0.04); } // Klik mekanik halus 
    playAlarm() { this._playTone(150, 'square', 0.4, 0.05, 0.3); } // Peringatan getar rendah (bass)
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.body.addEventListener('click', () => { if(qAudio.ctx && qAudio.ctx.state === 'suspended') qAudio.ctx.resume(); }, {once: true});
    initSplash();
    ...
    // DRM PERBAIKAN: BEBAS SWITCH APP. SENSOR SEBELUMNYA TELAH DIMUSNAHKAN.
    function applyTacticalDRM(drm) {
        ...
        // HANYA MENDETEKSI GESTUR 3 JARI UNTUK RED STRIKE
        if (drm.redStrike) {
            document.addEventListener('touchstart', (e) => { if (e.touches.length >= 3) triggerStrobe(); }, { passive: false });
        }
    }

    // PEREKAM SUARA: LOGIKA POINTER EVENT MUTLAK (ANTI-MACET, ANTI-FREEZE)
    let mediaRecorder;
    let audioChunks = [];
    ...
    const startRecording = async (e) => {
        ...
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                // MENCEGAH SPAM: Jika rekam di bawah 0.8 detik, buang. Jangan kirim file rusak.
                if (duration < 800) {
                    console.log("[SISTEM] Rekaman terlalu singkat. Dibatalkan secara senyap.");
                    return;
                }
                ...
                try {
                    ...
                    // RESET UI MUTLAK BAHKAN JIKA GAGAL UPLOAD
    ...
    // Menggunakan Pointer Event: 1 Sensor membaca sentuhan DAN klik mouse dengan stabil
    btnVoiceNote.addEventListener('pointerdown', startRecording);
    btnVoiceNote.addEventListener('pointerup', stopRecording);
    btnVoiceNote.addEventListener('pointercancel', stopRecording); // Jika scroll keluar tombol
    btnVoiceNote.addEventListener('pointerleave', stopRecording);
