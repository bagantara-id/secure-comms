...
class QuantumAudio {
    ...
    playBoot() { this._playTone(400, 'sine', 1.0, 0.05, 0.8); } 
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. EKSTRAKSI URL MUTLAK SEBELUM DIHAPUS
    const roomId = new URLSearchParams(window.location.search).get('session');
    let currentDRM = {}; 
    let strikeCount = 0;
    let isBurned = false;

    if (!roomId) return window.location.replace('https://www.google.com');
    ...
    const btnInitCrypto = document.getElementById('btnInitCrypto');
    ...
    // ==========================================
    // INISIALISASI GERBANG UTAMA
    // ==========================================
    btnInitCrypto.addEventListener('click', async () => {
        btnInitCrypto.innerText = "MENGOTENTIKASI...";
        qAudio.playBoot();

        // Mengikat identitas perangkat dan menghancurkan URL
    ...
    const triggerStrobe = () => {
        ...
        if (strikeCount === 1) {
            ...
            // DOM Scrambling: Hancurkan visual chat stream
    ...
    function renderMessage(msgId, data) {
        ...
        // PERBAIKAN: Sanitasi XSS di Klien
