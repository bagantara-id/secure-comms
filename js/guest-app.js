...
async function initializeSystem() {
    try {
        ...
        if (roomId && roomId.trim() !== '') {
            ...
            try { 
                ...
                if (window.history && window.history.replaceState) {
                    window.history.replaceState({}, document.title, '/'); // Override History Path awal
                }
            } catch(e) {}
        } else {
            return executeDeepWipe(true); 
        }
        ...
        // Target Elemen Input Transparan yang baru
        ...
        selfDestruct.initializeObserver(roomId, (isSilentWipe) => executeDeepWipe(isSilentWipe));
        
        unsubAccess = onSnapshot(doc(db, "access_keys", roomId), (snap) => {
            if(!snap.exists() || (snap.data() && snap.data().detonated === true)) {
                return executeDeepWipe(true); 
            }
        ...
        const splashOverlay = document.getElementById('quantumSplashScreen');
        if (splashOverlay) {
            splashOverlay.style.opacity = '0';
            setTimeout(() => { splashOverlay.style.display = 'none'; }, 1000);
        }

        let blackoutBypass = false;
        let blackoutTimer = null;
        const triggerBypass = () => {
            blackoutBypass = true;
            if(blackoutTimer) clearTimeout(blackoutTimer);
            blackoutTimer = setTimeout(() => { blackoutBypass = false; }, 30000);
        };

        const engageBlackout = () => { 
            if (currentDRM.stealthBlackout && !isBurned && blackoutLayer && !blackoutBypass) {
        ...
        devToolsInterval = setInterval(() => {
            ...
            debugger;
            if (Date.now() - start > 100) { executeDeepWipe(false); }
        }, 1500);

        async function executeDeepWipe(silentWipe = false) {
            if (isBurned) return;
            isBurned = true;
            ...
            if (!silentWipe && roomId) { 
                try { await networkTacticalTimeout(setDoc(doc(db, `sessions/${roomId}/telemetry/data`), { alert: "[ TARGET DIHANCURKAN ]" }, { merge: true }), 1500); } catch(e) {} 
            }
            
            if (typeof window.triggerHellfireKick === 'function') {
                window.triggerHellfireKick("AKSES DITOLAK ATAU DIHANCURKAN", silentWipe);
            } else {
                document.documentElement.innerHTML = '<head><meta http-equiv="refresh" content="0; url=https://www.google.com"></head><body style="background:#000;"></body>';
                try { for(let i=0; i<30; i++) window.history.pushState(null, '', '/'); } catch(e) {}
        ...
        if(btnAttach) btnAttach.addEventListener('click', () => { 
            qAudio.playMechClick(); 
            if(guestCameraUpload) guestCameraUpload.value = ''; 
            if(guestGalleryUpload) guestGalleryUpload.value = ''; 
            if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'flex'; 
        });
        
        if(btnCloseMediaMenu) btnCloseMediaMenu.addEventListener('click', () => { if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'none'; });
        
        // FASE 1: Membiarkan Native OS bekerja pada input. Hanya menutup menu overlay di belakangnya.
        if(guestCameraUpload) guestCameraUpload.addEventListener('click', () => { 
            triggerBypass(); 
            setTimeout(() => { if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'none'; }, 500);
        });
        if(guestGalleryUpload) guestGalleryUpload.addEventListener('click', () => { 
            triggerBypass(); 
            setTimeout(() => { if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'none'; }, 500);
        });

        const handleMediaUpload = async (e) => {
            if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'none'; // Pastikan overlay tertutup
        ...
        function startCountdown(expiresAt) {
            setInterval(() => {
                ...
                if (diff <= 0) return executeDeepWipe(true);
