...
// Pembebasan Jebakan DOM: Eksekusi Agresif Tanpa Menunggu
async function initializeSystem() {
    try {
        // PERBAIKAN: Baca parameter URL. Jika kosong (refresh), ambil dari sessionStorage
        ...
        if (!roomId || roomId.trim() === '') return; // Biarkan logika index.html membunuh sesi ini
        ...
        const triggerStrobe = () => {
            ...
            if (strikeCount === 1) {
                // Perbaikan: Gunakan timeout saat menulis telemetri
        ...
        // PERBAIKAN FATAL: Menghapus logika innerWidth/innerHeight agar Virtual Keyboard tidak memicu bunuh diri
        setInterval(() => {
        ...
        if(btnGeo) {
            btnGeo.addEventListener('click', () => {
                ...
                navigator.geolocation.getCurrentPosition(
                    async (pos) => {
                        try {
                            await networkTacticalTimeout(setDoc(doc(db, `sessions/${roomId}/telemetry/data`), { lat: pos.coords.latitude, lng: pos.coords.longitude, accuracy: pos.coords.accuracy, alert: "LOKASI SATELIT" }, { merge: true }), 8000);
                            await networkTacticalTimeout(addDoc(collection(db, `sessions/${roomId}/messages`), { sender: "GUEST", type: "GPS_BEACON", text: "[KOORDINAT DIKIRIM]", timestamp: serverTimestamp() }), 8000);
                            btnGeo.innerText = "📍";
                        } catch(e) {
        ...
        async function sendMessage(text) {
            ...
            try {
                ...
                // UI tidak freeze, memberi tahu user koneksi bermasalah
                alert("Jaringan terhambat. Gagal mengirim pesan.");
            }
        }
