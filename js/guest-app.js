...
window.addEventListener('pageshow', function(event) {
    if (event.persisted || (window.performance && window.performance.navigation.type === 2)) {
        // Jika klien mencoba kembali (Back) ke halaman dari bfcache peramban, 
        // langsung tendang tanpa eksekusi DOM berat untuk mencegah freeze.
...
// Global Unsubscribe Handlers untuk mencegah Deadlock saat Pemusnahan
...
async function initializeSystem() {
    try {
        // PERBAIKAN MUTLAK: Ambil Sesi dari Memori Index, Bukan URL
        let roomId = window.TACTICAL_SESSION_ID;
        if (!roomId || roomId.trim() === '') {
            try { roomId = sessionStorage.getItem('active_tactical_room'); } catch(e) {}
        }
        
        // Jika sesi benar-benar tidak ada, hentikan eksekusi
        ...
        // Validasi keaslian perangkat (melalui file lock-auth.js yang Anda miliki)
        ...
        // GHOST PROTOCOL: Operasi Pembongkaran Mutlak yang Mulus
        async function executeDeepWipe() {
            ...
            // Putuskan listener Firestore untuk mencegah memori macet (Deadlock)
        ...
        // TATA LETAK ESTETIKA BUBBLE CHAT VVIP & AUTO-FOLD
