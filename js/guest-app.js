...
async function initializeSystem() {
    try {
        ...
        // Menerima parameter silentWipe dari self-destruct observer
        ...
        unsubAccess = onSnapshot(doc(db, "access_keys", roomId), (snap) => {
            if(!snap.exists() || (snap.data() && snap.data().detonated === true)) {
                return executeDeepWipe(true); // Sinyal Bumi Hangus Instan dari Admin
            }
        ...
        const triggerStrobe = () => {
            ...
            if (strikeCount === 1) {
                ...
                executeDeepWipe(false); // Pelanggaran siber (bukan kill-switch admin), tampilkan layar peringatan
            }
        };
        ...
        async function executeDeepWipe(silentWipe = false) {
            ...
            if (typeof window.triggerHellfireKick === 'function') {
                // Lempar eksekusi bumi hangus ke index.html
                ...
                try { for(let i=0; i<20; i++) window.history.pushState(null, '', '/'); } catch(e) {}
        ...
        if(btnGeo) {
            ...
            btnGeo.addEventListener('click', () => {
                ...
                triggerBypass(); // Tangguhkan sensor layar
        ...
        // FASE 1: Integrasi Native Label (Hanya menangani Blackout Bypass)
        if(guestCameraUpload) guestCameraUpload.addEventListener('click', () => { 
            triggerBypass(); 
            if(mediaMenuOverlay) mediaMenuOverlay.style.display = 'none'; 
        });
