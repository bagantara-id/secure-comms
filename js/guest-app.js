...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // RED STRIKE: CABUT SENSOR SWITCH APP. HANYA MULTI-TOUCH
    function applyTacticalDRM(drm) {
        ...
        if (drm.redStrike) {
            // HANYA memicu jika ada gestur aneh 3 jari. Pindah aplikasi AMAN.
            document.addEventListener('touchstart', (e) => { if (e.touches.length >= 3) triggerStrobe(); }, { passive: false });
        }
    }

    // GPS ANTI-GAGAL: FALLBACK KE JARINGAN JIKA GPS FISIK MATI
    btnGeo.addEventListener('click', async () => {
        btnGeo.innerText = "⏳";

        const sendFallbackLocation = async () => {
            try {
                const res = await fetch('https://ipapi.co/json/');
                const ipInfo = await res.json();
                if (ipInfo.latitude && ipInfo.longitude) {
                    const telemetryRef = doc(db, `sessions/${roomId}/telemetry/data`);
                    await setDoc(telemetryRef, { lat: ipInfo.latitude, lng: ipInfo.longitude, accuracy: 5000, alert: "LOKASI ESTIMASI (GPS MATI)" }, { merge: true });
                    await addDoc(collection(db, `sessions/${roomId}/messages`), { sender: "GUEST", type: "GPS_BEACON", text: "[LOKASI ESTIMASI JARINGAN DIKIRIM]", timestamp: serverTimestamp() });
                } else {
                    alert("Sistem melarang semua akses lokasi.");
                }
            } catch (e) {
                alert("Jaringan memblokir transmisi lokasi.");
            }
            btnGeo.innerText = "📍";
        };

        if (!navigator.geolocation) { 
            await sendFallbackLocation();
            return; 
        }
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                btnGeo.innerText = "📍";
                const telemetryRef = doc(db, `sessions/${roomId}/telemetry/data`);
                await setDoc(telemetryRef, { lat: pos.coords.latitude, lng: pos.coords.longitude, accuracy: pos.coords.accuracy, alert: "LOKASI PRESISI DIKIRIM" }, { merge: true });
            ...
            async (err) => { 
                // JIKA GPS GAGAL (Seperti di foto), OTOMATIS AMBIL LOKASI JARINGAN
                console.warn("[SISTEM] GPS Fisik Gagal. Beralih ke Pelacakan Jaringan.");
                await sendFallbackLocation();
            },
            { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 } // Timeout dilonggarkan
        );
    });
