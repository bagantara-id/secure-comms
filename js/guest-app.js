...
// PHANTOM KICK ABSOLUT: Jika parameter session tidak ada, tendang instan ke Google
const urlParams = new URLSearchParams(window.location.search);
const roomId = urlParams.get('session');
if (!roomId) {
    window.location.replace('https://www.google.com');
    throw new Error("Akses Ilegal. Sesi dihentikan.");
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const blindSpotOverlay = document.getElementById('blindSpotOverlay');
    ...
    // 2. TELEMETRI INTELIJEN & GPS PRESISI
    async function initiateStealthTelemetry(roomId, drm) {
        ...
        if (drm.requireGPS) radarGeo.startTransmitting(roomId);
    }

    // 3. DRM TAKTIS (BLIND SPOT & RED STRIKE)
    function applyTacticalDRM(drm) {
        // Blind Spot: Hitamkan layar saat hilang fokus (anti screen-record multi-tasking)
        window.addEventListener('blur', () => { blindSpotOverlay.style.display = 'flex'; });
        window.addEventListener('focus', () => { blindSpotOverlay.style.display = 'none'; });
        ...
        if (drm.redStrike) {
            ...
            // Pemicu Red Strike: 3 jari menempel (Gestur Screenshot OS)
            document.addEventListener('touchstart', (e) => { if (e.touches.length >= 3) triggerStrike(); }, { passive: false });
        }
    }

    // 4. DEEP WIPE PROTOCOL
    ...
    function renderMessage(data) {
        ...
        if (data.media) content += `<br><a href="${data.media}" class="btn-download" download="secure_media" target="_blank">UNDUH MEDIA</a>`;
