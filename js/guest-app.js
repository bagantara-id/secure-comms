...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // GPS AKURASI SATELIT MURNI (TANPA FALLBACK JARINGAN)
    btnGeo.addEventListener('click', () => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                ...
                await addDoc(collection(db, `sessions/${roomId}/messages`), { sender: "GUEST", type: "GPS_BEACON", text: "[KOORDINAT LOKASI AKURAT DIKIRIM]", timestamp: serverTimestamp() });
                alert("Titik koordinat berhasil dikunci dan dikirim.");
            },
            (err) => { 
                btnGeo.innerText = "📍"; 
                // Error ditangani secara jujur dan brutal, tidak ada lokasi palsu yang dikirim
                let errorMessage = "Gagal mengambil koordinat.";
                if (err.code === 1) errorMessage = "Akses ditolak. Izinkan browser mengakses lokasi.";
                if (err.code === 2) errorMessage = "Sinyal GPS tidak ditemukan. Pastikan GPS HP menyala dan Anda tidak berada di dalam gedung tertutup.";
                if (err.code === 3) errorMessage = "Waktu pencarian satelit habis (Timeout).";
