...
async function initializeSystem() {
    try {
        ...
        async function executeDeepWipe() {
            ...
            document.title = "Google Search";
            // MENCABUT pushState DI SINI AGAR TIDAK DUPLIKASI HISTORI
        ...
        function renderMessage(msgId, data) {
            ...
            if (data.media) {
                if (data.type === "AUDIO") {
                    ...
                    // PERBAIKAN MUTLAK: TOMBOL UNDUH MEDIA UNTUK KLIEN KETIKA ADMIN MENGIRIM GAMBAR
                    if (data.sender === "ADMIN") {
                        content += `<button onclick="forceDownload('${data.media}', this)" style="margin-top: 8px; background: transparent; border: 1px dashed var(--quantum-cyan); color: var(--quantum-cyan); padding: 6px 12px; border-radius: 4px; font-size: 10px; font-weight: bold; cursor: pointer; transition: all 0.3s; width: 100%;">[ UNDUH MEDIA ]</button>`;
                    }
                }
            }
