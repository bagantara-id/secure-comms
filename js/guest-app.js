...
// Mengaktifkan AudioContext pada interaksi pertama klien (karena tombol dihilangkan)
document.body.addEventListener('pointerdown', () => {
    if (qAudio.ctx && qAudio.ctx.state === 'suspended') qAudio.ctx.resume();
}, { once: true });

document.addEventListener('DOMContentLoaded', async () => {
    // 1. INTERSEPSI URL MURNI (GHOST PROTOCOL - BUMI HANGUS)
    const roomId = new URLSearchParams(window.location.search).get('session');
    
    if (!roomId || roomId.trim() === '') {
        // Hancurkan DOM seketika
        document.body.innerHTML = '<div style="background:#000; width:100vw; height:100vh; position:fixed; z-index:2147483647; inset:0;"></div>';
        // Bakar jejak lokal
        try { localStorage.clear(); sessionStorage.clear(); } catch(e) {}
        // Timpa riwayat dan tendang ke Google
    ...
    // Mencegah klik kanan dan interaksi seleksi teks
    ...
    // ==========================================
    // INISIALISASI GERBANG UTAMA (OTOMATIS)
    // ==========================================
    const validatedData = await lockAuth.validateAndLock(roomId);
    
    // Jika tidak valid, lockAuth sudah mengurus tendangan keluar
    if (!validatedData) return; 

    currentDRM = validatedData.drm || {};
    
    // Beri waktu 3 detik bagi klien untuk membaca teks rahasia, lalu buka portal
    setTimeout(() => {
        splash.style.opacity = '0';
        setTimeout(() => splash.style.display = 'none', 800);
    }, 3000);

    // Menanamkan Protokol Pemusnahan Modular
    selfDestruct.initializeObserver(roomId, executeDeepWipe);
    
    // Membangun Saraf Telepati Real-Time ke Panel Admin
    onSnapshot(doc(db, "access_keys", roomId), (snap) => {
        if(!snap.exists()) return executeDeepWipe();
        currentDRM = snap.data().drm || {};
        btnGeo.style.display = currentDRM.allowManualGPS ? 'flex' : 'none';
    });

    // Memantau status Phantom Lock
    onSnapshot(doc(db, "sessions", roomId), (snap) => {
        if (snap.exists() && snap.data().isLocked) {
            chatInput.disabled = true; chatInput.placeholder = "[ SISTEM TERKUNCI ]";
            btnSend.disabled = true; qAudio.playAlarm();
        } else {
            chatInput.disabled = false; chatInput.placeholder = "Ketik pesan...";
    ...
    // ==========================================
    // TAKTIK BRUTAL 1: STEALTH BLACKOUT
    // ==========================================
    const engageBlackout = () => { if (currentDRM.stealthBlackout && !isBurned) blackoutLayer.style.display = 'flex'; };
    const disengageBlackout = () => { blackoutLayer.style.display = 'none'; };
    ...
    // ==========================================
    // TAKTIK BRUTAL 2: RED STRIKE & DOM SCRAMBLING
    // ==========================================
    const triggerStrobe = () => {
        ...
        if (strikeCount === 1) {
            ...
            chatStream.style.filter = "blur(12px) contrast(300%) hue-rotate(180deg)";
            chatStream.style.opacity = "0.2";
            
            strobeLayer.style.display = 'block';
            redStrikeScreen.style.display = 'flex';
            
            let isBlack = false;
            const strobeInterval = setInterval(() => { strobeLayer.style.background = isBlack ? '#fff' : '#000'; isBlack = !isBlack; }, 40);
            
            setTimeout(() => { 
                clearInterval(strobeInterval); 
                strobeLayer.style.display = 'none'; 
                redStrikeScreen.style.display = 'none'; 
                chatStream.style.filter = "none";
                chatStream.style.opacity = "1";
            }, 3000);
        } else {
            executeDeepWipe();
        }
    ...
    // ==========================================
    // TAKTIK BRUTAL 3: DEVTOOLS EXECUTIONER
    // ==========================================
    setInterval(() => {
    ...
    async function sendMessage(text) {
        ...
        chatInput.value = ''; qAudio.playMechClick();
    }
    ...
    document.getElementById('btnPing').addEventListener('click', () => { qAudio.playMechClick(); addDoc(collection(db, `sessions/${roomId}/messages`), { sender: "GUEST", type: "PING", timestamp: serverTimestamp() }); });

    // ==========================================
    // TRANSMISI MEDIA (GAMBAR & VIDEO)
    // ==========================================
